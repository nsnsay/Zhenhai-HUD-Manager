...
export const useGsiStore = defineStore('gsi', () => {
	...
	function connect(options: GsiConnectOptions = {}): void {
		const { url = 'http://127.0.0.1:1469', autoRefresh = false, reconnection = true, frameSync = false } = options;
 ...
 * 会在组件作用域销毁时自动取消监听。
 */
export function useGsiEvent<K extends GsiEventName>(event: K, listener: GsiEventListener<K>): () => void {
