	...
	digest = (raw: CSGORaw): CSGO | null => {
		...
		if (this.preEmitTransform) {
			...
			if (focusedSteamId && Array.isArray(data.players)) {
				data.player = data.players.find(player => player.steamid === focusedSteamId) ?? data.player;
			}
		...
		if (last.bomb && data.bomb) {
			...
			if ((last.bomb.state === 'planting' || last.bomb.state === 'carried') && data.bomb.state === 'planted') {
