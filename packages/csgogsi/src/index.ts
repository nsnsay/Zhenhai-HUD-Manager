...
export type PreEmitTransform = (data: CSGO) => CSGO;
...
class CSGOGSI {
  ...
  private preEmitTransform: PreEmitTransform | null = null;
   ...
   * 设置事件触发前的数据转换器。
   *
   * 这个转换会在 CSGOGSI 内部触发 roundEnd / bombPlant / mvp / phaseChange 等事件之前执行。
   * 因此事件携带的 Player / Team / Score / Map 数据都会是转换后的数据。
   *
   * 注意：
   * 这里只支持同步转换。
   */
  setPreEmitTransform = (transform: PreEmitTransform | null) => {
    this.preEmitTransform = transform;
    return this;
  };
  ...
  digest = (raw: CSGORaw): CSGO | null => {
    ...
    if (
      examplePlayerCT &&
      examplePlayerCT.observer_slot &&
      examplePlayerT &&
      examplePlayerT.observer_slot
    ) {
    ...
    const observed =
      players.find((player) => raw.player && player.steamid === raw.player.steamid) || null;
    ...
    for (const player of players) {
      ...
      const damageForRound = damage.filter(
        (damageEntry) => damageEntry.round < currentRoundForDamage,
      );
     ...
     * 第二套方案核心：
     *
     * 在 CSGOGSI 触发事件之前执行 pipeline / transform。
     * 这样后续：
     * - roundEnd
     * - matchEnd
     * - overtime
     ...
     * 等事件拿到的都是增强后的数据。
     */
    if (this.preEmitTransform) {
      try {
        const transformed = this.preEmitTransform(data);

        if (transformed) {
          data = transformed;
        }
      } catch (error) {
        console.error("[CSGOGSI] preEmitTransform error", error);
      }

      /**
       * 如果 pipeline 替换了 team / player / bomb.player 等引用，
       * 这里尽量把后续事件会用到的引用同步到增强后的对象。
       */
      teamCT = data.map.team_ct;
      teamT = data.map.team_t;

      if (data.phase_countdowns.phase === "timeout_ct") {
        data.phase_countdowns.timeout_team = teamCT;
      } else if (data.phase_countdowns.phase === "timeout_t") {
        data.phase_countdowns.timeout_team = teamT;
      }

      const focusedSteamId = data.player?.steamid;

      if (focusedSteamId && Array.isArray(data.players)) {
        data.player =
          data.players.find((player) => player.steamid === focusedSteamId) ?? data.player;
      }

      const bombPlayerSteamId = data.bomb?.player?.steamid;

      if (bombPlayerSteamId && data.bomb && Array.isArray(data.players)) {
        data.bomb.player =
          data.players.find((player) => player.steamid === bombPlayerSteamId) ?? data.bomb.player;
      }
    }
    ...
    if (last.bomb && data.bomb) {
      ...
      if (
        (last.bomb.state === "planting" || last.bomb.state === "carried") &&
        data.bomb.state === "planted"
      ) {
    ...
    const mvp =
      data.players.find((player) => {
        const previousData = last.players.find(
          (previousPlayer) => previousPlayer.steamid === player.steamid,
        );
  ...
  digestMIRV = (raw: RawKill | RawHurt, eventType = "player_death"): DigestMirvType => {
    if (eventType === "player_death") {
      ...
      const kill: KillEvent = {
        killer:
          killer ||
          (data.weapon === "trigger_hurt" || data.weapon === "worldspawn" ? victim : null),
        victim,
