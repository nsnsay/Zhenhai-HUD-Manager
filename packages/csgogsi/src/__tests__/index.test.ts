...
test("data > rounds: proper parser in 1st half", () => {
  ...
  const data = GSI.digest(
    createGSIPacket({ map: { round: 3, team_ct: { score: 2 }, team_t: { score: 1 } } }),
  );
...
test("data > bomb: undefined player if doesnt exist or not specified", () => {
  ...
  const playerDoesntExist = GSI.digest(
    createGSIPacket({ bomb: { player: "notExistingPlayerId" } }),
  );
...
test("event > round: mvp listener", () => {
  ...
  GSI.digest(
    createGSIPacket({ allplayers: { "76561199031036917": { match_stats: { mvps: 5 } } } }),
  );
  GSI.digest(
    createGSIPacket({ allplayers: { "76561199031036917": { match_stats: { mvps: 6 } } } }),
  );
...
test("event > round: mvp listener #2", () => {
  ...
  const packet = createGSIPacket({
    allplayers: { "76561198238326438": { match_stats: { mvps: 5 } } },
  });
  ...
  GSI.digest(packet);
  GSI.digest(
    createGSIPacket({ allplayers: { "76561198238326438": { match_stats: { mvps: 6 } } } }),
  );
...
test("event > match: ended listener, CT wins", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_ct: { score: 15 }, team_t: { score: 10 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
...
test("event > match: ended listener, T wins", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_t: { score: 15 }, team_ct: { score: 10 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
...
test("event > overtime: emitted when the score gets tied at regulationMR", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_ct: { score: 12 }, team_t: { score: 11 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
...
test("event > overtime: not emitted when the score is not tied", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_ct: { score: 12 }, team_t: { score: 10 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
      map: { team_ct: { score: 12 }, team_t: { score: 11 }, phase: "live" },
...
test("event > overtime: emitted only once per tie", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_ct: { score: 12 }, team_t: { score: 12 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
...
test("event > overtime: follows a custom regulationMR", () => {
  ...
  GSI.digest(
    createGSIPacket({ map: { team_ct: { score: 15 }, team_t: { score: 14 }, phase: "live" } }),
  );
  GSI.digest(
    createGSIPacket({
...
test("event > kill: get correct killer", () => {
  ...
  assert.equal(callback.mock.calls.length, 1);
  assert.equal(
    response && "killer" in response && response?.killer && response?.killer.steamid,
    "76561199031036917",
  );
});
