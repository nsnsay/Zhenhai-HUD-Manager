...
async function autoDetectCs2() {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      toast.add({ title: "CS2 Found", description: res.data, icon: "i-lucide-check", color: "success" });
    } else {
      toast.add({ title: "Not Found", description: "Please select the folder manually.", color: "warning", icon: "i-lucide-alert-triangle" });
    }
  } catch (e) {
...
async function createTournament(): Promise<boolean> {
  if (!tournamentName.value.trim()) {
    toast.add({ title: "Validation Error", description: "Tournament name is required.", color: "warning" });
    return false;
  }
...
async function installCfg() {
  ...
  try {
    ...
    if (res.success) {
      ...
      toast.add({ title: "CFG Installed", description: "GSI configured successfully.", icon: "i-lucide-check", color: "success" });
    } else {
...
async function finishWizard() {
  ...
  try {
    const existing = extrasStore.items.find(
      (i) => i.configType === "app-settings",
    ) as ExtrasRecord | undefined;
    const settings = existing?.settings
      ? JSON.parse(JSON.stringify(toRaw(existing.settings)))
      : {};
...
<template>
  <UModal v-model:open="open" :close="false" :dismissible="false" title="Welcome to Zhenhai HUD Manager"
        ...
        <template #content>
          <div class="min-h-[240px] flex flex-col justify-center py-6">
            ...
            <div v-if="currentStep === 0" class="space-y-4">
              <p class="text-sm text-muted">We need to locate your Counter-Strike 2 installation to configure Game State
                Integration (GSI).</p>
              <UFormField label="CS2 Root Directory" required>
                <div class="flex gap-2 w-full">
                  <UInput v-model="cs2Path"
              ...
              <UButton label="Auto Detect via Steam" icon="i-lucide-radar" :loading="isProcessing"
                @click="autoDetectCs2" class="w-fit" />
            </div>

            <div v-else-if="currentStep === 1" class="space-y-4">
              <p class="text-sm text-muted">Create your first tournament to start managing teams, players, and matches.
              </p>
              <UFormField label="Tournament Name" required>
                <UInput v-model="tournamentName" placeholder="e.g., ESL Pro League Season 20" class="w-full" />
              </UFormField>
              <UFormField label="Description (Optional)">
                <UTextarea v-model="tournamentDesc" placeholder="Brief description..." class="w-full" :rows="3" />
              </UFormField>
            </div>

            <div v-else-if="currentStep === 2" class="space-y-4">
              <p class="text-sm text-muted">Install the GSI configuration file to your CS2 directory so the game can
                send data to this app.</p>
              <div class="p-4 bg-elevated/50 rounded-lg border border-default/50 text-xs font-mono break-all">
                {{ cs2Path }}/game/csgo/cfg/gamestate_integration_zhenhai.cfg
              </div>
              <div class="flex items-center gap-3">
                <UButton v-if="!cfgInstalled" label="Install CFG File" icon="i-lucide-download" :loading="isProcessing"
                  @click="installCfg" />
                <UBadge v-else label="Installed Successfully" color="success" variant="subtle"
                  icon="i-lucide-check-circle-2" size="lg" />
              </div>
            </div>
            ...
            <div v-else-if="currentStep === 3" class="text-center space-y-6 py-8">
              ...
              <div class="pt-4">
                <a href="https://github.com/nsnsay/Zhenhai-HUD-Manager" target="_blank"
    ...
    <template #footer>
      <UButton v-if="currentStep > 0" label="Back" color="neutral" variant="ghost" @click="handleBack" />
      <div v-else />

      <div class="flex gap-2">
        <UButton v-if="currentStep < items.length - 1" label="Next" color="primary"
          :disabled="isProcessing || (currentStep === 0 && !cs2Path) || (currentStep === 1 && !tournamentName.trim())"
          @click="handleNext" />
        <UButton v-else label="Finish & Close" color="primary" :loading="isProcessing" @click="finishWizard" />
      </div>
    </template>
  </UModal>
</template>
