...
async function handleToggleLive(match: MatchRecord) {
  ...
  if (isCurrentlyLive) {
    ...
    const liveMatchs = matchItems.value.filter((m) => m.isLive === true && m.id !== match.id);
  ...
  <div class="w-full h-full flex flex-col relative">
    ...
    <div class="w-full h-(--ui-components-header-height) flex flex-row items-end">
      ...
      <div class="flex items-center justify-end gap-2">
        <div
          class="rounded-lg bg-elevated/45 p-1 ring-1 ring-white/5 shadow-sm backdrop-blur-md"
          size="xs"
        >
          <UButton
            size="md"
    ...
    <div class="flex-1 overflow-auto mt-4">
      ...
      >
        <div
          v-for="match in filteredMatchs"
          :key="match.id"
          @contextmenu="handleMatchContextMenu(match)"
        ...
        >
          ...
          <div class="flex flex-row items-center justify-between gap-2.5 mb-3 w-full">
            <div class="flex items-center gap-2.5">
              <UAvatar
                :src="getTeamLogo(match.matchTeamA)"
                size="xl"
            ...
            <div class="flex items-center gap-2.5">
              ...
              <UAvatar
                :src="getTeamLogo(match.matchTeamB)"
                size="xl"
                class="rounded-xl bg-zinc-800/80 ring-1 ring-white/10 p-1"
              />
            </div>
          ...
          <div
            class="flex items-center justify-between gap-2 text-[13px] text-muted mb-1 flex-wrap"
          >
            <template v-for="(map, index) in match.matchVeto" :key="index">
              <UBadge
                v-if="map.mapVetoType === 'pick' || map.mapVetoType === 'decider'"
                color="neutral"
                variant="outline"
                size="md"
                :label="map.mapName"
          ...
          >
            ...
            >
              <UButton
                :icon="match.isLive ? 'i-lucide-square' : 'i-lucide-play'"
                size="xs"
                :color="match.isLive ? 'error' : 'success'"
                :variant="match.isLive ? 'solid' : 'ghost'"
                @click="handleToggleLive(match)"
        ...
        >
          <template #teamA-cell="{ row }">
            <div class="flex items-center gap-2">
              <UAvatar
                :src="getTeamLogo((row.original as MatchRecord).matchTeamA)"
                size="md"
                class="bg-zinc-700 p-1"
              />
              <span class="font-medium">{{
                getTeamName((row.original as MatchRecord).matchTeamA)
              }}</span>
            </div>
          </template>
          <template #teamB-cell="{ row }">
            <div class="flex items-center gap-2">
              <UAvatar
                :src="getTeamLogo((row.original as MatchRecord).matchTeamB)"
                size="md"
                class="bg-zinc-700 p-1"
              />
              <span class="font-medium">{{
                getTeamName((row.original as MatchRecord).matchTeamB)
              }}</span>
            </div>
          </template>
          ...
          <template #actions-cell="{ row }">
            <div class="group flex gap-1">
              <UButton
                :icon="(row.original as MatchRecord).isLive ? 'i-lucide-square' : 'i-lucide-play'"
                size="xs"
                :color="(row.original as MatchRecord).isLive ? 'error' : 'success'"
                :variant="(row.original as MatchRecord).isLive ? 'solid' : 'ghost'"
                @click="handleToggleLive(row.original as MatchRecord)"
                aria-label="Toggle Live"
              />
              <UButton
                icon="i-lucide-pencil"
