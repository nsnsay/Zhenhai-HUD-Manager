...
const api: WindowAPI = {
  ...
  setWindowMaterial: (material) => ipcRenderer.invoke('main-window:set-background-material', material),
  ...
  onOverlayLifecycle: (callback: (state: string) => void) => {
    const subscription = (_event: Electron.IpcRendererEvent, state: string): void => callback(state)
