...
type TeamDbExtension = TeamInfo & TeamFormData & {
...
function createTeamEnricher(dbService: DatabaseService): GsiMiddleware {
  return (data: CSGO) => {
    ...
    const dbTeams = (
      teamsResult.success && teamsResult.data ? teamsResult.data : []
    ) as unknown as DbTeam[]
    const dbPlayers = (
      playersResult.success && playersResult.data ? playersResult.data : []
    ) as unknown as DbPlayer[]
    const dbMatchs = (
      matchsResult.success && matchsResult.data ? matchsResult.data : []
    ...
    const matchDbTeam = (gsiTeamName: string, teamSteamIds: string[]) => {
      ...
      for (const steamId of teamSteamIds) {
        ...
        if (
          dbTeam &&
          (dbTeam.teamGameName === gsiTeamName || dbTeam.teamName === gsiTeamName)
        )
          return dbTeam
      }
