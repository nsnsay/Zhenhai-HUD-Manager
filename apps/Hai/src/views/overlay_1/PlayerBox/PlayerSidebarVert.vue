...
<template>
  <div class="flex relative w-auto h-auto">
    <div
      :class="[{ isDead: player.isDead, 'outline-2': player.isFocused }]"
      class="player-info-box flex flex-col w-80 h-22 rounded-(--hai-radius) relative"
    >
      <div
        class="absolute inset-0 w-full flex flex-row z-10 overflow-hidden group-[&.T]:flex-row-reverse"
      >
        <div class="w-26 h-full relative z-20">
          <PlayerAvatar
            class="w-full mask-b-from-30% absolute inset-0 z-10 aspect-square scale-125 translate-y-3"
            :player="player"
            size="both"
          />
        </div>
        <div class="flex-1 flex flex-col w-full h-full group-[&.T]:items-end">
          <div
            class="flex flex-row justify-start items-center w-full h-7 gap-1 group-[&.T]:flex-row-reverse"
          >
        ...
        <div
          v-if="!player.isDead"
          class="w-20 h-full flex flex-col items-center justify-end relative"
        >
          <Weapons
            class="flex items-center justify-center"
            :player="player"
            :isolate-image="player.team.side === 'T'"
            size="64px"
          />
          <Grenades
      ...
      <div class="flex-1 w-full h-7 absolute left-0 top-0 overflow-hidden rounded-(--hai-radius)">
        <HealthBar
          :custom-class-name="['z-1! bg-pri/30! group-[&.T]:flex group-[&.T]:justify-end']"
