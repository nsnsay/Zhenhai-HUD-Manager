...
function pickMeta(map: MapPickVeto) {
  ...
  const opponent =
    map.mapPickTeam === teamA.value?.id
      ? teamLabel(teamB.value?.id)
      : teamLabel(teamA.value?.id);
...
<template>
  <div v-if="selectedMaps.length > 0"
    ...
    <TransitionGroup name="veto-card" tag="div" class="flex w-full flex-col items-end gap-1">
      <div v-for="(map, index) in selectedMaps" :key="`${map.mapName}-${index}`"
        ...
        <div
          ...
          :style="mapBackground(map.mapName) ? { backgroundImage: `url(${mapBackground(map.mapName)})` } : {}" />
